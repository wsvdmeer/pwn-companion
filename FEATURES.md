...
An Android companion for [Pwnagotchi](https://github.com/jayofelony/pwnagotchi): a single-screen **terminal console** that turns your phone into the Pwnagotchi's brain-on-the-go — live screen, GPS, a deterministic on-device personality voice, and a deauth "where to hunt" advisor that helps it catch **more handshakes**.
...
## On-device personality (fully local — no cloud, no model)
- **No language model** — the voice is a curated per-franchise corpus selected deterministically by mood + a persistent franchise, with live-data slots filled from real capture stats. Instant, offline, weightless; nothing leaves the phone. *(Earlier versions ran a ~491 MB Qwen2.5-0.5B via llama.cpp; it was removed once the personality moved into the corpus — same voice, none of the download/RAM/battery cost.)*
...
- **One blended cult-movie voice** — no picker; a hacker-gremlin persona drawing on **21 film & game worlds** (Evil Dead, Star Wars, Matrix/Mr Robot, Harry Potter, Terminator, Tron, Jurassic Park, Alien, RoboCop, Blade Runner, WarGames, Hackers, Portal/GLaDOS, Predator, HAL 9000, Cyberpunk 2077, SHODAN, Mad Max, Ghostbusters, Back to the Future, The Thing), but committing to **exactly one franchise per line** (pinned per utterance, never blended mid-sentence). The current world is captioned under the mirrored e-ink. Its tone (hyped · grumpy · weary · deadpan) is chosen live by the emergent mood — happy after a catch, grumpy when it's hot/blind/dry, weary when idle.
- **Grounded, data-backed lines** — the session recap folds real numbers into the corpus (catches this session, crackable/cracked counts, best channel, the AP that keeps escaping, how it feels) via slot templates, so the pet can never state a wrong count or channel; while hunting it volunteers its best spot on its own.
...
- **Speaks on the device's own screen** — instead of the pwnagotchi's stock, repeating `voice.py` quips, the app pushes a rolling pool of fresh in-character lines keyed to the device's native voice moments (normal · bored · sad · angry · excited · grateful · lonely · handshakes · deauth · assoc · motivated · demotivated). The plugin splices them straight into the e-ink speech bubble — filled both by real event reactions and a round-robin refresh while connected (weighted toward `normal`, the state shown most) — so the pet says something different every time. In **MANUAL mode** it even replaces the stock "Kicked N stations / Got N handshakes" recap screen with a recap built from the app's richer data (catches this session, cracked/crackable, best channel, mood). Lines are **length-calibrated to the tiny e-ink** (one short clause, ~a dozen–40 chars, matching the stock voice). Falls back to its own stock voice cleanly whenever the app is disconnected or quiet; nothing to configure on the pwnagotchi.

## Deauth advisor — catch more handshakes
- The hunt advisor tells you **where to hunt now** — ranks channels by the device's own per-channel captures + live client density (clients = deauth targets). All analysis is phone-side and deterministic; only genuine problems surface (as `[ alerts ]`), and the pet voices them on its e-ink.
