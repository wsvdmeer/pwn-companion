...
Android companion app for [Pwnagotchi](https://github.com/jayofelony/pwnagotchi). Connects to the Pwnagotchi over Bluetooth PAN, receives live screen captures, provides GPS location, and runs an on-device AI that reacts to WiFi hunting events in real time — and, crucially, **advises where to hunt next** so the device catches more handshakes.
...
- gives the pet a genuine **AI personality voice** — generated by a small on-device language model, shown both in the app *and* spliced onto the pwnagotchi's own screen;
...
<p>
  <img src="docs/screenshot-ai.png" width="47%" alt="AI pet card: mood, evolution stage, streaming replies, and the hunt/recap/status intents" />
  <img src="docs/screenshot-learning.png" width="47%" alt="Deauth advisor line and per-channel learning bars" />
</p>

*Left: the AI pet card — live disposition, evolution stage, and the "ask your pet" intents. Right: the deauth advisor + channel-learning readout.*
...
**Quickest — prebuilt APK:** grab the APK from the [**Releases** page](../../releases) and sideload it (enable "install unknown apps"). Note it's a **debug build** (debug-signed, for testing — not the Play Store); requires **Android 10+ on an arm64 device**, and the first launch downloads the ~491 MB AI model over Wi-Fi. To build it yourself instead:
...
- **Android NDK + CMake ≥ 3.22.1** — the app builds **llama.cpp from C++ source** (`app/src/main/cpp`) into `libllama-jni.so` for `arm64-v8a`. Install both via the SDK Manager. *The first build compiles llama.cpp and is slow (several minutes); later builds are cached.*
...
**First launch** downloads the AI model (Qwen2.5 0.5B Instruct Q4_K_M GGUF, ~491 MB) over WiFi, shown with a terminal ASCII progress bar; a version bump sweeps the old file automatically.
...
### Talking to the pet (fact-backed intents)

The pet reacts on its own to real events, but under **"ask your pet:"** in the `[ AI ]` section you can also prompt it. A row of tappable **intents** hands the model *pre-computed, correct facts* to phrase, so it can never invent a wrong number or channel:

- **`hunt`** (AUTO) — voices the deauth hunt advisor (see below).
- **`poke`** (MANUAL) — a quick in-character quip.
- **`recap`** — a data-grounded session digest: caught this session, best channel, busiest time, how it feels.
- **`status`** — a short in-character status check-in.

There's deliberately **no free-text box** (a 0.5B model is weak at open Q&A) — instead `recap` folds in the answers you'd ask for (catches, crackable/cracked counts, best spot, the AP that keeps escaping, how it feels), and the pet **volunteers** its best hunting spot on its own while hunting. An **`ai feed`** log records recent lines + what triggered each (reaction, emotion, RL reward, milestone, advisor) so you can see the AI features firing.

Replies **stream token-by-token** as the model generates them (real llama.cpp streaming via a per-token JNI callback — not a fake type-out), so the first words appear immediately. A refusal gate buffers the opening clause and vets it before anything reaches the screen.
...
The mood/tier decide *what* the line carries and *how it's coloured*; a per-line franchise pick keeps each line coherent. The LLM gets the persona + a tone directive + the pinned franchise; the curated fallback pool is keyed by `category × tone` with single-franchise lines, so even the canned voice never blends. Adding worlds later is just extending the franchise list. If the on-device model ever refuses the roleplay ("Sorry, I can't assist…") it's detected and replaced with the in-character voice so a refusal never reaches the screen.

---

## Deauth Advisor — where to hunt next

The whole point of a Pwnagotchi is to **deauth clients and capture handshakes**, so the app's headline AI feature is a `[ advisor ]` line that tells you where to point it. **All the analysis is done on the phone (Kotlin); the LLM only phrases it** — so the recommendation is always correct and the pet just adds flavor.
...
- **Mission alerts** — deauth-blocking problems, in the error color: `blind` (monitor interface / antenna dead), `running hot` (thermal throttle), `APs but 0 clients` (nothing to deauth here — move), and dry-spell detection. New alerts are also voiced proactively by the pet.

Tapping **`hunt`** makes the pet speak the same advice in-character.
...
Net: a richer AI (and a real LLM voice) **without** adding a gram of load, heat, or battery drain to the device whose radio is already stretched thin.
...
|---|---|---|
| What "AI" is | **A2C reinforcement learning** — a neural net (`MlpLstmPolicy`, TensorFlow/stable-baselines) | **UCB1 bandit** + heuristic hill-climb (classic online algorithms) **+ a small LLM for voice** |
...
| Voice | static `voice.py` templates | LLM-generated, mood- & franchise-driven — on the phone **and** on the pet's own e-ink |
| Footprint | heavy (torch/TF) — **removed** in the jayofelony fork | **zero** extra load on the Pi; compute + model live on the phone |
...
Built on / credits: [Pwnagotchi (jayofelony fork)](https://github.com/jayofelony/pwnagotchi) · [llama.cpp](https://github.com/ggerganov/llama.cpp) (on-device inference) · Qwen2.5-0.5B (voice model).
