...
**Requires Android 10+ (API 29)** on an arm64 device, plus a [Pwnagotchi](https://github.com/jayofelony/pwnagotchi) running the [`bt-tether`](https://github.com/wsvdmeer/pwnagotchi-plugins) + `pwn-companion.py` plugins. The `pwn-companion` plugin is now **available in [pwnstore](https://pwnstore.org/)** for one-tap install (see [GETTING_STARTED.md](GETTING_STARTED.md)).
